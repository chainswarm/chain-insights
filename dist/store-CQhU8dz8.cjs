...
var store_exports = /* @__PURE__ */ require_chunk.__exportAll({
	CaseStore: () => CaseStore,
	casesRoot: () => casesRoot,
...
const CaseStore = {
	...
	async loadContext(id) {
		...
		const { frontmatter } = require_frontmatter.parseFrontmatter(await (0, node_fs_promises.readFile)(node_path.default.join(dir, "case.md"), "utf8"));
		const tags = (frontmatter["tags"] ?? "").split(",").filter(Boolean);
		const { SessionStore } = await Promise.resolve().then(() => require("./session-BT7VpbAd.cjs")).then((n) => n.session_exports);
		const { DossierStore } = await Promise.resolve().then(() => require("./dossier-BXy57V4-.cjs")).then((n) => n.dossier_exports);

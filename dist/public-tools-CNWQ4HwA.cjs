...
const require_graph_normalizer = require("./graph-normalizer-l-XcE-a2.cjs");
...
async function runFundFlowProbe(remoteClient, _config, options) {
	...
	if (paths) {
		const { generateInlineGraphHtml } = await Promise.resolve().then(() => require("./html-generator-DeTV9DFI.cjs")).then((n) => n.html_generator_exports);

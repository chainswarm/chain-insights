...
const ParamSpecSchema = zod.object({
	name: zod.string(),
	type: zod.enum([
		"string",
		"number",
...
parse(markdown, resolvedParams) {
	...
	const steps = body.split(/^## /m).slice(1).map((section, i) => {
		...
		const tool = applyTemplate(extractFencedBlock(section, "tool") ?? "", resolvedParams);
		const rawParams = parseParamsBlock(extractFencedBlock(section, "params") ?? "");
		const params = {};
		for (const [k, v] of Object.entries(rawParams)) params[applyTemplate(k, resolvedParams)] = applyTemplate(v, resolvedParams);
		return {
			index: i + 1,
			label,
			tool,
...
//#endregion
exports.PlaybookParser = PlaybookParser;

...
const casesRoot = require_active.activeCasesRoot;
function caseDir(id) {
...
const CaseStore = {
	...
	async loadContext(id) {
		...
		const { frontmatter } = require_frontmatter.parseFrontmatter(await (0, node_fs_promises.readFile)(node_path.default.join(dir, "case.md"), "utf8"));
		const tags = (frontmatter["tags"] ?? "").split(",").filter(Boolean);
		const { SessionStore } = await Promise.resolve().then(() => require("./session-W2OnrECv.cjs"));
		const { DossierStore } = await Promise.resolve().then(() => require("./dossier-dzeQT1i3.cjs"));

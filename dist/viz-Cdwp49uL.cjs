...
const require_data_extractor = require("./data-extractor-DLJ0qfvG.cjs");
const require_html_generator = require("./html-generator-9xvA43R8.cjs");
...
async function generateVisualization(opts) {
	...
	if (opts.dataFile) {
		...
		const { extractGraphFromJson } = await Promise.resolve().then(() => require("./data-extractor-DLJ0qfvG.cjs")).then((n) => n.data_extractor_exports);
		rawData = extractGraphFromJson(parsed);
	} else if (opts.caseId) {
		const { extractGraphFromCase } = await Promise.resolve().then(() => require("./data-extractor-DLJ0qfvG.cjs")).then((n) => n.data_extractor_exports);

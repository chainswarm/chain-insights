...
import { n as truncateGraph } from "./data-extractor-4vEOW80r.mjs";
import { r as writeVizHtml, t as generateHtml } from "./html-generator-BrlZCf3u.mjs";
...
async function generateVisualization(opts) {
	...
	if (opts.dataFile) {
		...
		const { extractGraphFromJson } = await import("./data-extractor-4vEOW80r.mjs").then((n) => n.t);
		rawData = extractGraphFromJson(parsed);
	} else if (opts.caseId) {
		const { extractGraphFromCase } = await import("./data-extractor-4vEOW80r.mjs").then((n) => n.t);
...
//# sourceMappingURL=viz-CHzLOPBR.mjs.map
...
import { n as truncateGraph } from "./data-extractor-Brh_zZfn.mjs";
...
async function generateVisualization(opts) {
	...
	if (opts.dataFile) {
		...
		const { extractGraphFromJson } = await import("./data-extractor-Brh_zZfn.mjs").then((n) => n.t);
		rawData = extractGraphFromJson(parsed);
	} else if (opts.caseId) {
		const { extractGraphFromCase } = await import("./data-extractor-Brh_zZfn.mjs").then((n) => n.t);
...
//# sourceMappingURL=viz-CH1fYwmB.mjs.map
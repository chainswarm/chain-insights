...
program.command("access-key").description("Configure Chain Insights Graph test access key mode").addCommand(new Command("set").description("Use a Chain Insights Graph test access key without x402 payments").argument("<key>", "Test access key").option("--endpoint <url>", "Chain Insights Graph endpoint").action(async (key, opts) => {
	try {
...
program.command("setup").description("Configure external MCP clients").addCommand(new Command("claude-code").alias("claude").description("Install Claude Code skills and register the MCP proxy").action(() => {
	runInstaller("--claude");
})).addCommand(new Command("codex").description("Install Codex skills and register the MCP proxy").action(() => {
	runInstaller("--codex");
})).addCommand(new Command("hermes").description("Install Hermes skills and register the MCP proxy").action(() => {
	runInstaller("--hermes");
}));

...
program.command("config").description("Read or write configuration values").addCommand(new Command("get").argument("<key>", "Config key to read").action(async (key) => {
	...
	if (key === "walletPrivateKey") {
		try {
			const { setWalletPrivateKey } = await import("./wallet-3OUnh-O2.mjs").then((n) => n.s);
...
program.command("wallet").description("Manage the local Base USDC payment wallet").addCommand(new Command("import").description("Import a Base payment wallet").argument("<private-key>", "0x-prefixed EVM private key").action(async (privateKey) => {
	...
	try {
		const { getWalletAccount } = await import("./tools-C2UrlOUn.mjs").then((n) => n.c);
	...
	try {
		const { getWalletBalanceText } = await import("./tools-C2UrlOUn.mjs").then((n) => n.c);
	...
	try {
		const { formatWalletReadiness, parsePaymentApprovalUnits, prepareWalletForPaidCalls } = await import("./tools-C2UrlOUn.mjs").then((n) => n.c);
	...
	try {
		const { buildTopupInfo, getWalletAccount } = await import("./tools-C2UrlOUn.mjs").then((n) => n.c);
...
program.command("mcp").description("Interact with the Chain Insights MCP endpoint").allowExcessArguments(false).addCommand(new Command("networks").description("List supported graph networks, capability layers, retention, and freshness").option("--json", "Print raw capability JSON").action(async (opts) => {
	...
	try {
		await withGraphMcpClient("chain-insights-cli-address-risk", async (client) => {
			...
			const { addressRisk } = await import("./public-tools-BKNH0BGv.mjs");
	...
	try {
		...
		await withGraphMcpClient("chain-insights-cli-track-funds", async (client, config) => {
			...
			const { trackFunds } = await import("./public-tools-BKNH0BGv.mjs");
	...
	try {
		...
		await withGraphMcpClient("chain-insights-cli-scam-topology", async (client, config) => {
			const { scamTopology } = await import("./public-tools-BKNH0BGv.mjs");
	...
	try {
		await withGraphMcpClient("chain-insights-cli-stake-insights", async (client) => {
			const { stakeInsights } = await import("./public-tools-BKNH0BGv.mjs");

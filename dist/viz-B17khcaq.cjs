...
const require_data_extractor = require("./data-extractor-ByB1PXGZ.cjs");
...
async function generateVisualization(opts) {
	...
	if (opts.dataFile) {
		...
		const { extractGraphFromJson } = await Promise.resolve().then(() => require("./data-extractor-ByB1PXGZ.cjs")).then((n) => n.data_extractor_exports);
		rawData = extractGraphFromJson(parsed);
	} else if (opts.caseId) {
		const { extractGraphFromCase } = await Promise.resolve().then(() => require("./data-extractor-ByB1PXGZ.cjs")).then((n) => n.data_extractor_exports);

...
import { o as safeFilename, r as graphToCanvas } from "./canvas-Cn-maEIh.mjs";
...
async function scaffoldVault(options) {
	...
	for (const dir of VAULT_DIRS) await mkdir(join(workspaceRoot, dir), { recursive: true });
	const filesWritten = [];
...
async function readCaseEvidence(caseId) {
	const evidenceDir = join(workspaceOutputPaths().casesRoot, caseId, "evidence");
	const files = await readdir(evidenceDir).catch(() => []);
	const docs = [];
	for (const filename of files.filter((file) => file.endsWith(".md")).sort()) {
		const { frontmatter, body } = parseFrontmatter(await readFile(join(evidenceDir, filename), "utf8"));
		docs.push({
	...
	return {
		nodes: canvas.nodes.map((node) => {
			...
			if (node.type === "file" && entityIndex) {
				...
				if (graphNode) return {
					...node,
					file: `Entities/${safeFilename(entityLabelForGraphNode(graphNode, Number(entityIndex) - 1))}`
				};
			}
			return node;
		}),
...
//#endregion
export { assertNoVaultFileCollisions, refreshCaseVault, scaffoldVault };

//# sourceMappingURL=vault-z35Dohdq.mjs.map
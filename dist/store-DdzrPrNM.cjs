...
const require_output_root = require("./output-root-CncCLcN-.cjs");
...
const CaseStore = {
	...
	async loadContext(id) {
		...
		const { frontmatter } = require_frontmatter.parseFrontmatter(await (0, node_fs_promises.readFile)(node_path.default.join(dir, "case.md"), "utf8"));
		const tags = (frontmatter["tags"] ?? "").split(",").filter(Boolean);
		const { SessionStore } = await Promise.resolve().then(() => require("./session-C6ZrMG5k.cjs"));
		const { DossierStore } = await Promise.resolve().then(() => require("./dossier-Cc85w3oy.cjs"));

...
import { n as truncateGraph } from "./data-extractor-Cojxgpsw.mjs";
...
async function generateVisualization(opts) {
	...
	if (opts.dataFile) {
		...
		const { extractGraphFromJson } = await import("./data-extractor-Cojxgpsw.mjs").then((n) => n.t);
		rawData = extractGraphFromJson(parsed);
	} else if (opts.caseId) {
		const { extractGraphFromCase } = await import("./data-extractor-Cojxgpsw.mjs").then((n) => n.t);
...
//# sourceMappingURL=viz-D-_9WxI5.mjs.map
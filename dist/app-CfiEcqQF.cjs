...
function createApp() {
	...
	app.get("/artifacts/:artifactId/graph.json", async (c) => {
		...
		const { workspaceOutputPaths } = await Promise.resolve().then(() => require("./output-root-BtL2lJgv.cjs")).then((n) => n.output_root_exports);
		const paths = workspaceOutputPaths();
		const graphPath = node_path.default.resolve(paths.artifactsRoot, artifactId, "graph.json");
		if (!withinRoot(paths.artifactsRoot, graphPath)) return c.json({ error: "Invalid artifact ID" }, 400);
		if (!await realPathWithinRoot(paths.artifactsRoot, graphPath)) return c.json({ error: "Graph artifact not found" }, 404);
		try {

...
const require_data_extractor = require("./data-extractor-D49FZiRQ.cjs");
...
async function generateVisualization(opts) {
	...
	if (opts.dataFile) {
		...
		const { extractGraphFromJson } = await Promise.resolve().then(() => require("./data-extractor-D49FZiRQ.cjs")).then((n) => n.data_extractor_exports);
		rawData = extractGraphFromJson(parsed);
	} else if (opts.caseId) {
		const { extractGraphFromCase } = await Promise.resolve().then(() => require("./data-extractor-D49FZiRQ.cjs")).then((n) => n.data_extractor_exports);

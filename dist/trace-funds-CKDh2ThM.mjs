import { n as findActiveWorkspace } from "./active-DGDIabsm.mjs";
...
async function ensureDirs(root) {
	await mkdir(path.join(root, ".chain-insights", "schema"), {
		recursive: true,
		mode: 448
	});
	await mkdir(path.join(root, "reports", "graphs"), {
		recursive: true,
		mode: 448
	});
	await mkdir(path.join(root, "reports", "tables"), {
		recursive: true,
		mode: 448
	});
}
...
async function traceFunds(remoteClient, config, options) {
	...
	await writeFile(reportPath, buildMarkdownReport(seedAddress, network, flows, graphPath, schemaResult.filePath), { mode: 384 });
	if (options.caseId) {
...
//# sourceMappingURL=trace-funds-CKDh2ThM.mjs.map
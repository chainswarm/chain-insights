...
async function normalizeRemoteToolResult(result, config, toolName = "remote-graph") {
	...
	if (graphPayload) {
		const { writeGraphReport } = await Promise.resolve().then(() => require("./graph-reports-C9M5Rma0.cjs"));
		const { ensureArtifactServer } = await Promise.resolve().then(() => require("./artifact-server-BT38qyUf.cjs"));
...
async function createProxy() {
	...
	const { createConfiguredGraphMcpFetch, resolveGraphMcpEndpoint } = await Promise.resolve().then(() => require("./client-Dc0MXuij.cjs")).then((n) => n.client_exports);

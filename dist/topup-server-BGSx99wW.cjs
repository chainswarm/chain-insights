...
const require_tools = require("./tools-keOvDHi8.cjs");
...
const USDC = ${JSON.stringify(require_tools.USDC_ADDRESS)};
const CHAIN_ID = ${JSON.stringify(require_tools.BASE_CHAIN_HEX)};
...
async function handleRequest(req, res, address, url) {
	...
	if (parsedUrl.pathname === "/api/balance") {
		sendJson(res, 200, { balance_usdc: await require_tools.getBalanceUsdc(address) });
		return;
	}

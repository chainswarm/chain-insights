...
import { a as getBalanceUsdc, n as USDC_ADDRESS, r as buildTopupInfo, t as BASE_CHAIN_HEX } from "./tools-DlfsacMx.mjs";
...
<body>
  ...
  <script>
    ...
    const usdcAddress = ${JSON.stringify(USDC_ADDRESS)};
    const baseChainId = ${JSON.stringify(BASE_CHAIN_HEX)};
	...
	if (parsedUrl.pathname === "/api/balance") {
		try {
			sendJson(res, 200, {
				...
				network: "Base",
				token: "USDC",
				balance_usdc: await getBalanceUsdc(address)
			});
		} catch (err) {
			sendJson(res, 502, {
				wallet_address: address,
...
//# sourceMappingURL=topup-server-CDA6HQQs.mjs.map
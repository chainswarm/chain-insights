...
import { n as truncateGraph } from "./data-extractor-CCde6Tje.mjs";
...
async function generateVisualization(opts) {
	...
	if (opts.dataFile) {
		...
		const { extractGraphFromJson } = await import("./data-extractor-CCde6Tje.mjs").then((n) => n.t);
		rawData = extractGraphFromJson(parsed);
	} else if (opts.caseId) {
		const { extractGraphFromCase } = await import("./data-extractor-CCde6Tje.mjs").then((n) => n.t);
...
//# sourceMappingURL=viz-DtRIQFQu.mjs.map
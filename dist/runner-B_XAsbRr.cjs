...
const require_evidence = require("./evidence-jiHhjzKM.cjs");
...
async run(playbook, opts) {
	...
	await require_init.initSchema(conn);
	conn.closeSync();
	let caseId;
	if (opts.caseId) caseId = (await require_evidence.CaseStore.get(opts.caseId)).id;
	else {
		caseId = (await require_evidence.CaseStore.create({
	...
	const paymentFetch = require_client.createMcpFetchClient(config.mcpEndpoint);
	const client = new _modelcontextprotocol_sdk_client_index_js.Client({
		name: "chain-insights-playbook",
		version: "0.1.0"
	});
	await client.connect(new _modelcontextprotocol_sdk_client_streamableHttp_js.StreamableHTTPClientTransport(new URL(config.mcpEndpoint), { fetch: paymentFetch }));
	let evidenceCount = 0;
	try {

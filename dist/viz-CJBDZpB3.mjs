...
import { n as truncateGraph } from "./data-extractor-xnDE970X.mjs";
import { r as writeVizHtml, t as generateHtml } from "./html-generator-CuePLEU_.mjs";
...
async function generateVisualization(opts) {
	...
	if (opts.dataFile) {
		...
		const { extractGraphFromJson } = await import("./data-extractor-xnDE970X.mjs").then((n) => n.t);
		rawData = extractGraphFromJson(parsed);
	} else if (opts.caseId) {
		const { extractGraphFromCase } = await import("./data-extractor-xnDE970X.mjs").then((n) => n.t);
...
//# sourceMappingURL=viz-CJBDZpB3.mjs.map
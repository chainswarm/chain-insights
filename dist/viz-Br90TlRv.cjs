...
const require_data_extractor = require("./data-extractor-_2nyKVOi.cjs");
const require_html_generator = require("./html-generator-BwYmG5mJ.cjs");
...
async function generateVisualization(opts) {
	...
	if (opts.dataFile) {
		...
		const { extractGraphFromJson } = await Promise.resolve().then(() => require("./data-extractor-_2nyKVOi.cjs")).then((n) => n.data_extractor_exports);
		rawData = extractGraphFromJson(parsed);
	} else if (opts.caseId) {
		const { extractGraphFromCase } = await Promise.resolve().then(() => require("./data-extractor-_2nyKVOi.cjs")).then((n) => n.data_extractor_exports);

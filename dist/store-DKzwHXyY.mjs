...
import { n as workspaceOutputPaths } from "./output-root-DSl0xFAF.mjs";
...
const CaseStore = {
	...
	async loadContext(id) {
		...
		const { frontmatter } = parseFrontmatter(await readFile(path.join(dir, "case.md"), "utf8"));
		const tags = (frontmatter["tags"] ?? "").split(",").filter(Boolean);
		const { SessionStore } = await import("./session-CVoPhLrv.mjs");
		const { DossierStore } = await import("./dossier-DDAPHMD1.mjs");
...
//# sourceMappingURL=store-DKzwHXyY.mjs.map
...
import { t as activeCasesRoot } from "./active-DGDIabsm.mjs";
...
const casesRoot = activeCasesRoot;
function caseDir(id) {
...
const CaseStore = {
	...
	async loadContext(id) {
		...
		const { frontmatter } = parseFrontmatter(await readFile(path.join(dir, "case.md"), "utf8"));
		const tags = (frontmatter["tags"] ?? "").split(",").filter(Boolean);
		const { SessionStore } = await import("./session-gGpmcpDU.mjs");
		const { DossierStore } = await import("./dossier-DbK1v9kv.mjs");
...
//# sourceMappingURL=store-hkT5ChXC.mjs.map
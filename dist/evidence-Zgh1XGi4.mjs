import { t as getDb } from "./init-DYD_r6eL.mjs";
import { n as serializeFrontmatter, t as parseFrontmatter } from "./frontmatter-DiiF_6KA.mjs";
...
const CaseStore = {
	...
	async loadContext(id) {
		...
		const { frontmatter } = parseFrontmatter(await readFile(path.join(dir, "case.md"), "utf8"));
		const tags = (frontmatter["tags"] ?? "").split(",").filter(Boolean);
		const { SessionStore } = await import("./session-DFuQ3DhQ.mjs");
		const { DossierStore } = await import("./dossier-BXDpEYnB.mjs");
...
//# sourceMappingURL=evidence-Zgh1XGi4.mjs.map
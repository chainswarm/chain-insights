...
- Direct Chain Insights Graph `network_capabilities` exposes only the public `bittensor` semantic investigation network; it must not advertise alias/source databases or unsupported networks such as `bittensor_evm`, `base`, `ethereum`, or `tron`.

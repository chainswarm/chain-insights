...
- `devkit/`: deterministic local Bittensor graph backend (StarRocks facade tables, Memgraph, MemGQL/Memgraph Zero, and a devkit-only lite Go backend under `devkit/chain-insights-graph-devkit/`); the lite backend landed via PR #130, fixture parity via PRs #131/#132.
...
- Devkit backend exposes ONLY `network_capabilities`/`usage_status`/`graph_query`/`graph_query_batch` at `http://127.0.0.1:18012/mcp` and intentionally tracks production contract: two-tier query timeouts (live 10s, archive+facts 30s), live/archive capability sublayers, unmetered usage_status; `USE live_topology`→Memgraph, `USE archive_topology`/facts→StarRocks via Memgraph Zero. Tier detail is owned by `rbmk-system-knowledge`.
...
- Devkit clean-state cycle: `docker compose -f devkit/docker-compose.yml down -v --remove-orphans` then `up -d --build`; one-shot import/bootstrap services must exit 0 and starrocks/memgraph/memgql/chain-insights-graph-devkit must stay running.

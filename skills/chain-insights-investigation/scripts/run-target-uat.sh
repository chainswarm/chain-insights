...
cia mcp call graph_query_batch \
  network="${NETWORK}" \
  'queries=[{"id":"node_labels","query":"USE topology MATCH (a:Address) RETURN \"Address\" AS node_label, count(a) AS sample_count LIMIT 1"},{"id":"relationship_types","query":"USE topology MATCH (:Address)-[r:FLOWS_TO]->(:Address) RETURN \"FLOWS_TO\" AS rel_name, count(r) AS sample_count LIMIT 1"},{"id":"linked_sample","query":"USE topology MATCH (a:Address)-[l:LINKED]-(b:Address) RETURN a.address AS address, b.address AS linked_address, b.network AS linked_network LIMIT 20"},{"id":"flow_sample","query":"USE topology MATCH (:Address)-[r:FLOWS_TO]->(:Address) RETURN r.amount_usd_sum AS amount_usd_sum, r.tx_count AS tx_count LIMIT 20"}]' \
  > "${SCHEMA_RAW}"
...
cia mcp call graph_query_batch \
  network="${NETWORK}" \
  "queries=[{\"id\":\"address_profile\",\"query\":\"USE topology MATCH (a:Address {address: \\\"${TARGET_ADDRESS}\\\"}) RETURN a.address AS address, a.network AS network, a.labels AS labels, a.degree_in AS degree_in, a.degree_out AS degree_out LIMIT 1\"}]" \
  > "${RESULT_FILE}"

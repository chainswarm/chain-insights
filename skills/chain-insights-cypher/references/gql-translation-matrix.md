...
| --- | --- | --- | --- |
| Bounded variable-length | `-[:FLOWS_TO*1..3]->` | `-[:FLOWS_TO]->{1,3}` | live only — archive rejects at runtime (`WITH RECURSIVE` unsupported by warehouse); use fixed-hop batches |
...
- **Archive `{m,n}` fails at runtime** (translation emits `WITH
  RECURSIVE`, unsupported by the warehouse backend), and the archive
  inner-`WHERE` group form silently drops both the anchor and the
  predicate. Archive traversal = explicit fixed-hop patterns, always.
...
| Bounded `{m,n}` quantified path | ✅ | ❌ runtime reject (`WITH RECURSIVE` unsupported; probe-verified) — fixed-hop batches only |
...
3. **Archive traversal:** quantified paths do not run on the warehouse
   backend (probe-verified runtime reject). Fixed-hop `graph_query_batch`
   patterns are the only archive traversal form.

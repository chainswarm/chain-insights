...
export function generateTopupPage(walletAddress: string): string {
  ...
  const addressJson = JSON.stringify(walletAddress)
  const usdcJson = JSON.stringify(USDC_ADDRESS)
  const baseChainHexJson = JSON.stringify(BASE_CHAIN_HEX)
...
<body>
  <main>
    ...
    <section aria-labelledby="wallet-heading">
      ...
      <div class="address" id="walletAddress">${address}</div>
      <div class="row">
  ...
  <script>
    const recipient = ${addressJson};
    const usdcAddress = ${usdcJson};
    const baseChainId = ${baseChainHexJson};
...
export async function startTopupServer(
  ...
  const server = createServer((req, res) => {
    void handleRequest(req, res, address, activeServer?.url ?? 'http://127.0.0.1')
  })
  ...
  const url = await new Promise<string>((resolve, reject) => {
    ...
    server.listen(options.port ?? 0, '127.0.0.1', () => {
      ...
      if (!addressInfo || typeof addressInfo === 'string') {
        reject(new Error('Top-up server did not bind to a TCP port'))
        return
      }

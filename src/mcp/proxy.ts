...
const GRAPH_SCHEMA_HINTS = [
  ...
  '- FLOWS_TO properties are tx_count, amount_usd_sum, first_seen_timestamp, last_seen_timestamp, plus the optional swap stamp below. No other edge fields exist: tx ids, coverage, and averages come from USE facts or inline arithmetic.',
  '- Swap attribution rides on FLOWS_TO as seven optional properties whose names contain a dot, so every one must be backquoted: `swap.kind`, `swap.family`, `swap.deployment`, `swap.pool`, `swap.reason`, `swap.route_id`, `swap.interpreter_version`. An edge with no `swap.kind` was never part of a swap-shaped transaction. Topology only.',
  '- `swap.kind` is one of swap, swap_like or swap_unsplit, weakest claim last. swap means the complete route is proven: payer, recipient, assets, exact raw amounts and conservation. swap_like means the shape is a swap but the pool bytecode matches no reviewed family. swap_unsplit means the legs are real but could not be paired into one route; `swap.reason` names why, for example batch_partition_ambiguous or capture_missing. Never read swap_unsplit as "no swap happened".',
  '- `swap.family` names the protocol family (for example uniswap-v2, uniswap-v3, uniswap-v4), or unknown when the route names no pool fact. `swap.deployment` is official or clone:<factory address>: a clone is decoded with its family rules and is not a lesser result. `swap.pool` is the pool address and `swap.route_id` ties every leg of one route together.',
  '- Find the swap legs of an address: MATCH (a:Address {address: $addr})-[r:FLOWS_TO]-(b:Address) WHERE r.`swap.kind` IS NOT NULL RETURN b.address, r.`swap.kind`, r.`swap.family`, r.`swap.pool`, r.`swap.reason`, r.amount_usd_sum. Collect one whole route with MATCH (:Address)-[r:FLOWS_TO]-(:Address) WHERE r.`swap.route_id` = $route_id.',
  '- Detailed swap facts are separate topology nodes, keyed by transaction, and are not needed for ordinary tracing: (:DexTransaction)-[:HAS_DEX_ROUTE]->(:DexRoute), -[:HAS_DEX_POOL_FACT]->(:DexPoolFact) and -[:HAS_DEX_CONTRIBUTION]->(:DexPairContribution). DexTransaction carries network, block_hash, block_height, transaction_hash, transaction_index. DexRoute carries route_id, rule_id and the parallel input/output/fee/refund address, asset and raw-amount arrays, plus pool_ids. DexPoolFact carries protocol, pool_address and pool_key. Reach them with MATCH (t:DexTransaction {transaction_hash: $tx})-[:HAS_DEX_ROUTE]->(route:DexRoute) RETURN route.route_id, route.input_assets, route.output_assets, route.input_raw_amounts, route.output_raw_amounts.',
  '- Swap stamps and Dex nodes are a disposable reading of the warehouse, not a money flow: they create no edge and move no value. The amounts on a route are exact raw token quantities, not USD, and they are not interchangeable with FLOWS_TO amount_usd_sum.',

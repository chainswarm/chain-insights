...
const KNOWN_PUBLIC_TOOL_DESCRIPTIONS: Record<string, string> = {
  ...
  graph_query: 'Run a read-only GQL/Cypher query through the Chain Insights graph endpoint. Use USE topology for topology (address/FLOWS_TO/LINKED graph, unified recent+historical, plus the node risk_score/risk_level verdict) and USE facts for labels, features, and enrichment. Preserve full addresses exactly.',
...
const GRAPH_SCHEMA_HINTS = [
  ...
  '- The risk verdict lives on topology nodes (risk_score float, risk_level string). Provenanced label detail still comes from USE facts: (:Address)-[:HAS_LABEL]->(:AddressLabel) for label risk, (:Address)-[:HAS_FEATURE]->(:AddressFeature) for feature metrics.',
  '- Facts graph labels include Address, AddressLabel, AddressFeature, NeuronEndpoint, Hotkey, and IPAddress. Facts address keys match topology address values exactly.',
  ...
  '- graph_query uses the active Chain Insights graph endpoint. Select the graph with USE topology for topology (address/FLOWS_TO/LINKED graph, unified recent+historical, plus the node risk_score/risk_level verdict) and USE facts for labels, features, and enrichment; address is the node grain, not the topology name.',
...
function knownPublicToolInputSchema(toolName: string): ToolInputShape | null {
  switch (toolName) {
    ...
    case 'graph_query':
      return {
        query: z.string().min(1).describe('Read-only GQL/Cypher query. Use USE topology for topology (address/FLOWS_TO/LINKED graph, unified recent+historical, plus the node risk_score/risk_level verdict) and USE facts for labels, features, and enrichment.'),
        network: BITTENSOR_NETWORK_SCHEMA,
      }
  ...
  server.registerPrompt(
    ...
    async ({ network, query }) => promptResult(
      [
        ...
        '```',
        '',
        'Use USE topology for topology (address/FLOWS_TO/LINKED graph, unified recent+historical, plus the node risk_score/risk_level verdict) and USE facts for labels, features, and enrichment. If you need schema context, first run small discovery queries such as MATCH (a:Address) RETURN a.address AS address, keys(a) AS address_properties LIMIT 5 and MATCH (:Address)-[r:FLOWS_TO]->(:Address) RETURN keys(r) AS flow_properties LIMIT 5. Return the full address when available; never shorten addresses with ellipses.',
      ].join('\n'),
      'Graph query',
    ),
  )

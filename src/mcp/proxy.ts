...
const GRAPH_SCHEMA_HINTS = [
  'Graph query hints for network=bittensor (and network=bittensor_evm):',
  '- The graph is address-grain. The only topology money node label is Address, keyed by the raw chain-native address (SS58 for network=bittensor, 0x... for network=bittensor_evm), for example 0x1874a43d7c6d888f9eda3d22a3a49704e3cadb24. There is no separate identity key.',
  '- Address nodes carry address, network, labels, and is_exchange. (:Address)-[:LINKED]-(:Address) is an undirected ownership-overlay edge (basis derived/associated, plus confidence, source_event, declared_owner) asserting the two addresses are controlled by the same actor; it is the only edge that can connect a bittensor address to a bittensor_evm address. Enumerate LINKED neighbors with MATCH (a:Address {address: $addr})-[l:LINKED]-(b:Address) RETURN b.address, b.network, l.basis, l.confidence.',
  ...
  '- Archive topology labels include Address. Archived money-flow topology is represented as (:Address)-[:FLOWS_TO]->(:Address) relationships with period_granularity, period_start_date, and period_end_date, and the archive LINKED ownership-overlay uses the same (:Address)-[:LINKED]-(:Address) shape as live_topology.',

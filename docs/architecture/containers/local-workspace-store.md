...
Filesystem workspace containing cases, evidence manifests, dossiers, exports, and config.
**Technology:** Files

## Purpose
Provides durable, append-only local storage for investigation artifacts. Workspaces are plain directories (no database), enabling direct inspection with any editor or agent tooling. Stores reports (Markdown, graph HTML, table HTML), evidence (compact JSON, graph JSON), schemas (runtime topology captures), and exported handoff bundles.

## Components
- **Workspace Layout:** .chain-insights/workspace-root marker, reports/ (Markdown, HTML), reports/graphs/ (graph JSON), reports/tables/ (CSV, compact evidence JSON), artifacts/, entities/, sessions/, published/
...
## Data Flow
<- ciaCli: Writes local cases, evidence, config, and exports
...
## Invariants
- Active workspace detection: Current directory with `.chain-insights/workspace-root` marker, or nearest parent directory with marker

...
## Purpose
Exposes Chain Insights investigation tools to AI agents (Claude Desktop, ChatGPT, Codex) via stdio MCP transport. Proxies configured local tools (wallet_balance, meta_help) and remote graph tools (aml_*, graph_query*) to the Chain Insights Graph MCP endpoint. Supports workspace mode (artifact persistence) and stateless mode (proxy-only). Logs structured tool/topology events to mcp-proxy.jsonl.

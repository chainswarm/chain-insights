...
- **Graph Query Contract:** topology (unified recent + full historical FLOWS_TO topology, Address nodes with labels/risk/is_exchange, plus the LINKED ownership overlay), facts (labels, features, risk scores, assets, enrichment). Reads only; no CREATE/MERGE/SET/DELETE.

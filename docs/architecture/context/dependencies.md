...
- **Graph Query Contract:** live_topology (recent FLOWS_TO topology, Identity nodes with labels/risk/is_exchange), archive_topology (historical period-granular flows), facts (labels, features, risk scores, assets, enrichment). Reads only; no CREATE/MERGE/SET/DELETE.

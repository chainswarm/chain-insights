...
export CHAIN_INSIGHTS_GRAPH_MCP_ENDPOINT=http://127.0.0.1:8012/mcp
...
npm --prefix repos/infra/chain-insights run dev -- mcp networks --json \
  > "$EVIDENCE_DIR/chain-insights-networks.json"

# devkit MCP only serves graph primitive calls; Chain Insights owns this recipe.
npm --prefix repos/infra/chain-insights run dev -- mcp call aml_address_risk "address=${SEED_ADDRESS}" network=bittensor \

...
<!-- RBMK workflow:project-end -->

<!-- RBMK workflow:stack-start source:research/STACK.md -->
## Technology Stack
...
|------------|---------|---------|-----|------------|
| TypeScript | 6.0.x | Primary language | Type safety for complex investigation data models, npm distribution alignment, x402/viem ecosystem is TypeScript-first, RBMK workflow reference architecture uses TS. v6.0 is the current stable (final JS-based compiler before Go-native 7.0). | HIGH |
| Node.js | >= 22.0.0 | Runtime | Required by RBMK workflow reference (engines field), LTS support, native ESM, required by Commander 14.x. | HIGH |
### Distribution & CLI
| Technology | Version | Purpose | Why | Confidence |
|------------|---------|---------|-----|------------|
| npm | (bundled) | Package distribution | RBMK workflow-style `npx chain-insights` global install. Proven model from RBMK workflow reference. npm bin entry points for CLI commands. | HIGH |
| Commander.js | 14.0.x | CLI framework | 132K+ dependents, complete CLI solution (subcommands, options, help generation). RBMK workflow uses a custom CLI but for a new project Commander is the pragmatic choice -- less custom code, faster to ship. Requires Node >= 20. | HIGH |
### Build & Tooling
| Technology | Version | Purpose | Why | Confidence |
|------------|---------|---------|-----|------------|
...
| Vitest | 4.1.x | Test runner | Jest-compatible API, native ESM/TS support, fast watch mode. RBMK workflow reference uses Vitest. The built-in Node test runner lacks snapshot testing and DX polish needed for a project of this complexity. | HIGH |
### Local Server
| Technology | Version | Purpose | Why | Confidence |
|------------|---------|---------|-----|------------|
...
|-----------|--------|---------|-----|
| Case state | Markdown + YAML frontmatter | Investigation tracking | Human-readable, AI-agent-friendly, git-trackable. Same pattern as RBMK workflow's `docs/superpowers/`. No dependency needed -- Node.js fs. |
...
|----------|-------------|-------------|---------|
| Language | TypeScript | Python | npm distribution model, RBMK workflow architecture alignment, x402/viem/MCP SDKs are TypeScript-first. Python would mean pip distribution headaches and lose the RBMK workflow reference value. |
...
| Test Runner | Vitest | Jest | Jest has poor native ESM support, requires transforms for TypeScript, slower startup. Vitest is Jest-compatible but modern. RBMK workflow uses Vitest. |
...
| CLI Framework | Commander | Custom (RBMK workflow-style) | RBMK workflow rolls its own CLI via bin scripts. For a new project, Commander provides subcommands, help generation, and option parsing out of the box. Less custom code = faster to ship. |
...
## Architecture Alignment with RBMK workflow Reference
| RBMK workflow Pattern | Chain Insights Equivalent | Implementation |
|-------------|---------------------------|----------------|
...
| `docs/superpowers/` (state directory) | `.chain-insights/` (state directory) | Case files, dossiers, config, watch definitions. |
...
- RBMK workflow Reference: /home/aphex5/work/chain-insights/references/get-shit-done/package.json (local)
<!-- RBMK workflow:stack-end -->

<!-- RBMK workflow:conventions-start source:CONVENTIONS.md -->
## Conventions

Conventions not yet established. Will populate as patterns emerge during development.
<!-- RBMK workflow:conventions-end -->

<!-- RBMK workflow:architecture-start source:ARCHITECTURE.md -->
## Architecture

Architecture not yet mapped. Follow existing patterns found in the codebase.
<!-- RBMK workflow:architecture-end -->

<!-- RBMK workflow:skills-start source:skills/ -->
## Project Skills

No project skills found. Add skills to any of: `.claude/skills/`, `.agents/skills/`, `.cursor/skills/`, `.github/skills/`, or `.codex/skills/` with a `SKILL.md` index file.
<!-- RBMK workflow:skills-end -->

<!-- RBMK workflow:workflow-start source:RBMK workflow defaults -->
## RBMK workflow Workflow Enforcement

Before using Edit, Write, or other file-changing tools, start work through a RBMK workflow command so planning artifacts and execution context stay in sync.

Use these entry points:
- `/rbmk-quick` for small fixes, doc updates, and ad-hoc tasks
- `/rbmk-debug` for investigation and bug fixing
- `/rbmk-execute-phase` for planned phase work

Do not make direct repo edits outside a RBMK workflow workflow unless the user explicitly asks to bypass it.
<!-- RBMK workflow:workflow-end -->



<!-- RBMK workflow:profile-start -->
## Developer Profile

> Profile not yet configured. Run `/rbmk-profile-user` to generate your developer profile.
> This section is managed by `generate-claude-profile` -- do not edit manually.
<!-- RBMK workflow:profile-end -->

...
describe('deriveMergeOptionsFromQuery', () => {
  ...
  it('lifts count/sum/avg/min/max aliases into aggregateKeys', () => {
    const query = 'MATCH (a:Address) RETURN a.address AS address, count(a) AS n, sum(a.x) AS total, avg(a.y) AS mean, min(a.z) AS lo, max(a.z) AS hi'
    const options = deriveMergeOptionsFromQuery(query)
  ...
  it('classifies an invariant node property ORDER BY key as invariant', () => {
    const query = 'MATCH (t:Address) RETURN t.address AS address, t.risk_score AS risk_score ORDER BY t.risk_score DESC LIMIT 50'
    const options = deriveMergeOptionsFromQuery(query)
  ...
  it('classifies a non-invariant ORDER BY key (e.g. an edge sum) as merge-affected', () => {
    const query = 'MATCH (s)-[r:FLOWS_TO]->(t) RETURN t.address AS address, r.amount_usd_sum AS amount_usd_sum ORDER BY r.amount_usd_sum DESC LIMIT 200'
    const options = deriveMergeOptionsFromQuery(query)
  ...
  it('drops ORDER BY entirely when the sort key is itself an aggregate alias', () => {
    const query = 'MATCH (a:Address)-[r:FLOWS_TO]->(b) RETURN b.address AS address, sum(r.amount_usd_sum) AS usd ORDER BY usd DESC LIMIT 10'
    const options = deriveMergeOptionsFromQuery(query)
...
describe('mergeGraphRows — no-op guard', () => {
  it('passes a non-shard-tagged response through byte-identical (same array reference)', () => {
    const rows = [{ address: 'a', degree_in: 1 }, { address: 'b', degree_in: 2 }]
    const result = mergeGraphRows(rows, 'MATCH (a:Address) RETURN a.address AS address, a.degree_in AS degree_in')
  ...
  it('merges and strips __shard when rows are shard-tagged', () => {
    ...
    const result = mergeGraphRows(rows, 'MATCH (a:Address) RETURN a.address AS address, a.risk_score AS risk_score')
...
describe('applyShardMergeToBatchEntries', () => {
  const queries = [
    { id: 'q1', query: 'MATCH (a:Address) RETURN a.address AS address, a.risk_score AS risk_score ORDER BY a.risk_score DESC LIMIT 10' },
  ]

  it('is a no-op for entries whose rows carry no __shard (flag-off case)', () => {
    const entries: BatchQueryEntry[] = [{ id: 'q1', ok: true, results: [{ address: 'a', risk_score: 5 }] }]
  ...
  it('skips entries with ok:false or empty results without throwing', () => {
    const entries: BatchQueryEntry[] = [{ id: 'q1', ok: false, error: 'boom' }, { id: 'missing-query', ok: true, results: [{ __shard: 's1', x: 1 }] }]

...
import {
  addressRisk,
  buildMoneyTrailBlock,
  moneyTrailEndsQuery,
  moneyTrailIncidentQuery,
  moneyTrailSummarySentence,
...
describe('buildMoneyTrailBlock', () => {
  ...
  it('chooses the highest-value trail end as nearest_trail_end', () => {
    ...
    expect(block?.nearest_trail_end).toEqual({
      address: 'end-high',
      fact_type: 'cash_out',
      value: '500',
    })
...
describe('moneyTrailSummarySentence', () => {
  ...
  it('reads "sits on a money trail" for holding class', () => {
    const block = buildMoneyTrailBlock(
      [{ edge_class: 'holding', min_hop: 1, primary_seed: 'seedA', generation: 1 }],
      []
    )
    expect(moneyTrailSummarySentence(block!)).toContain('sits on a money trail')
  })
  ...
  it('reads "touched money-trail funds" for peripheral class', () => {
    const block = buildMoneyTrailBlock(
      [{ edge_class: 'peripheral', min_hop: 1, primary_seed: 'seedA', generation: 1 }],
      []
    )
        ...
        const results = queries.map((q) => {
          ...
          if (q.id === 'money_trail_incident') {
            return {
              ...
              ok: true,
              results: [
                { edge_class: 'transport', min_hop: 2, primary_seed: '5Seed', generation: 1 },
              ],
            }
          }

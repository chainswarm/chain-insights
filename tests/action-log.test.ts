...
describe('action log', () => {
  it('appends one JSON line per entry', async () => {
    ...
    await appendActionLog({ ts_ms: 1, tool: 'graph_query', args: { network: 'bittensor' }, outcome: 'ok', duration_ms: 12 })
    await appendActionLog({ ts_ms: 2, tool: 'aml_address_risk', args: {}, outcome: 'error', duration_ms: 3, error: 'boom' })
  ...
  it('is a no-op when unconfigured', async () => {
    ...
    await expect(appendActionLog({ ts_ms: 1, tool: 't', args: {}, outcome: 'ok', duration_ms: 1 })).resolves.toBeUndefined()
  })
  ...
  it('creates the parent directory', async () => {
    ...
    process.env.CIA_ACTION_LOG = file
    await appendActionLog({ ts_ms: 1, tool: 't', args: {}, outcome: 'ok', duration_ms: 1 })

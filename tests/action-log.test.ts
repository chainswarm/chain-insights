...
describe('action log', () => {
  it('appends one JSON line per entry', async () => {
    ...
    await appendActionLog({ timestamp: 1, tool: 'graph_query', args: { network: 'bittensor' }, outcome: 'ok', duration_ms: 12 })
    await appendActionLog({ timestamp: 2, tool: 'aml_address_risk', args: {}, outcome: 'error', duration_ms: 3, error: 'boom' })
  ...
  it('is a no-op when unconfigured', async () => {
    ...
    await expect(appendActionLog({ timestamp: 1, tool: 't', args: {}, outcome: 'ok', duration_ms: 1 })).resolves.toBeUndefined()
  })

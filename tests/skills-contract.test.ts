...
describe('shipped Chain Insights skills contract', () => {
  it('keeps investigation guidance on initialized workspaces and role-specific trace tools', () => {
    ...
    expect(skill).toContain('Live and archive topology are address-grain and graph-selected')
    ...
    expect(skill).toContain('Bittensor native Substrate/SS58 addresses such as')
    expect(skill).toContain('network=bittensor')
    expect(skill).toContain("Do not pass one network's addresses to the other")
    ...
    expect(skill).toContain('archive_linked_sample')
    expect(skill).toContain('USE archive_topology MATCH (a:Address)-[l:LINKED]-(b:Address)')
  ...
  it('keeps UAT guidance and scripts from treating home as an investigation output root', () => {
    ...
    expect(investigationUat).toContain('archive_linked_sample')
    expect(investigationUat).toContain('USE archive_topology MATCH (a:Address)-[l:LINKED]-(b:Address)')
  ...
  it('documents archive LINKED ownership-overlay projection wherever schema probes are shipped', () => {
    const readme = read('README.md')
    ...
    expect(combined).toContain('archive_linked')
    expect(combined).toContain('USE archive_topology MATCH (a:Address)-[l:LINKED]-(b:Address)')
  ...
  it('requires only current public proxy tools in Chain Insights Graph UAT', () => {
    ...
    expect(script).toContain('checking public live_topology address-grain edges')
    expect(script).toContain('public live_topology address-grain edges ok')
  ...
  it('ships generic Chain Insights Graph Cypher guidance with layer-aware schema capture', () => {
    ...
    expect(graphTools).toContain('archive_linked_sample')
    expect(graphTools).toContain('USE archive_topology MATCH (a:Address)-[l:LINKED]-(b:Address)')
  ...
  it('ships Bittensor-specific Cypher schema guidance for the two address-grain networks', () => {
    ...
    expect(skill).toContain('EVM-pallet `0x...`')
    expect(skill).toContain('Do not pass `network=bittensor` for a `0x...` address or vice versa')
    expect(skill).toContain('Use `network=bittensor_evm` for EVM-pallet')
    ...
    expect(skill).toContain('Live topology address grain')
    ...
    expect(readme).toContain('archive_linked')
    expect(readme).toContain('USE archive_topology MATCH (a:Address)-[l:LINKED]-(b:Address)')

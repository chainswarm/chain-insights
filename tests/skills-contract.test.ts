...
describe('shipped Chain Insights skills contract', () => {
  it('keeps investigation guidance on initialized workspaces and role-specific trace tools', () => {
    ...
    expect(skill).toContain('Live and archive topology are identity-grain but graph-selected')
    expect(skill).toContain('High-level `aml_*` tools may resolve addresses into identity-grain topology internally')
    expect(skill).toContain('public results, artifacts, and follow-up candidate lists must return member addresses')
    ...
    expect(skill).toContain('(:Identity)-[:FLOWS_TO]->(:Identity)')
  ...
  it('keeps UAT guidance and scripts from treating home as an investigation output root', () => {
    ...
    expect(skill).toContain('defaults to identity-grain topology')
    expect(skill).toContain('(:Identity)-[:HAS_ADDRESS]->(:Address)')
    ...
    expect(investigationUat).toContain('archive_member_address_sample')
    expect(investigationUat).toContain('USE archive_topology MATCH (i:Identity)-[:HAS_ADDRESS]->(m:Address)')
    expect(investigationUat).toContain('m.network AS member_network')
    expect(investigationUat).toContain('if [[ -d cases || -d Evidence ]]; then')
    expect(investigationUat).not.toContain('cia case ')
  })

  it('documents archive member-address network projection wherever schema probes are shipped', () => {
    const readme = read('README.md')
    ...
    expect(combined).toContain('archive_member_address_sample')
    expect(combined).toContain('USE archive_topology MATCH (i:Identity)-[:HAS_ADDRESS]->(m:Address)')
  ...
  it('requires only current public proxy tools in Chain Insights Graph UAT', () => {
    ...
    expect(script).toContain('direct-live-topology.json')
    expect(script).not.toContain('--tool-arg "topology_scope=identity"')
    expect(script).not.toContain('--tool-arg "topology_scope=address"')
    expect(script).toContain('checking public live_topology identity-grain edges')
    expect(script).toContain('public live_topology identity-grain edges ok')
    expect(script).toContain('direct-semantic-facts.json')
    expect(script).toContain('UAT_IDENTITY_KEY="${UAT_IDENTITY_KEY:-bittensor:0x')
    expect(script).toContain('checking facts identity query')
    expect(script).toContain('facts identity query ok')
    expect(script).not.toContain('direct-address-scope-rejection.json')
    expect(script).not.toContain('topology_scope must be identity')
  ...
  it('ships generic Chain Insights Graph Cypher guidance with layer-aware schema capture', () => {
    ...
    expect(examples).toContain('WHERE m.address STARTS WITH')
    ...
    expect(graphTools).toContain('archive_member_address_sample')
    expect(graphTools).toContain('USE archive_topology MATCH (i:Identity)-[:HAS_ADDRESS]->(m:Address)')
  ...
  it('ships Bittensor-specific Cypher schema guidance for the combined investigation network', () => {
    ...
    expect(skill).toContain('Bittensor EVM is not a separate public network')
    expect(skill).toContain('Identity')
    expect(skill).toContain('HAS_ADDRESS')
    expect(skill).toContain('archive_member_address_sample')
    expect(skill).toContain('USE archive_topology MATCH (i:Identity)-[:HAS_ADDRESS]->(m:Address)')
    expect(skill).toContain('amount_usd_sum')
    ...
    expect(skill).toContain('Live topology identity grain')
    expect(skill).toContain('Observed against the identity-serving contract on 2026-06-13')
    expect(skill).toContain('WHERE m.address STARTS WITH')
    expect(skill).toContain('Address-family census')
    expect(skill).toContain('references/memgraph-examples.md')
    ...
    expect(readme).toContain('archive_member_address')
    expect(readme).toContain('USE archive_topology MATCH (i:Identity)-[:HAS_ADDRESS]->(m:Address)')

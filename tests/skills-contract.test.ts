...
describe('shipped Chain Insights skills contract', () => {
  it('keeps investigation guidance on initialized workspaces and role-specific trace tools', () => {
    ...
    expect(skill).toContain('facts_linked_sample')
    expect(skill).toContain('USE facts MATCH (a:Address)-[l:LINKED]-(b:Address)')
    expect(skill).toContain('archive stays money-only')
  ...
  it('keeps UAT guidance and scripts from treating home as an investigation output root', () => {
    ...
    expect(graphUat).toContain('archive money-only FLOWS_TO ok')
    expect(graphUat).not.toContain('USE archive_topology MATCH (a:Address {address: \'\${UAT_ADDRESS}\'})-[l:LINKED]')
    ...
    expect(investigationUat).toContain('facts_linked_sample')
    expect(investigationUat).toContain('USE facts MATCH (a:Address)-[l:LINKED]-(b:Address)')
    expect(investigationUat).not.toContain('USE archive_topology MATCH (a:Address)-[l:LINKED]-(b:Address)')
  ...
  it('documents facts-tier LINKED ownership-overlay projection wherever schema probes are shipped (archive stays money-only)', () => {
    const readme = read('README.md')
    ...
    expect(combined).toContain('facts_linked')
    expect(combined).toContain('USE facts MATCH (a:Address)-[l:LINKED]-(b:Address)')
    expect(combined).not.toContain('USE archive_topology MATCH (a:Address)-[l:LINKED]-(b:Address)')
  ...
  it('ships generic Chain Insights Graph Cypher guidance with layer-aware schema capture', () => {
    ...
    expect(skill).toContain('facts_linked_sample')
    expect(skill).toContain('USE facts MATCH (a:Address)-[l:LINKED]-(b:Address)')
    expect(skill).toContain('archive_topology` stays money-only')
    ...
    expect(graphTools).toContain('facts_linked_sample')
    expect(graphTools).toContain('USE facts MATCH (a:Address)-[l:LINKED]-(b:Address)')
  ...
  it('ships Bittensor-specific Cypher schema guidance for the single public network with a property-split address space', () => {
    ...
    expect(readme).toContain('facts_linked')
    expect(readme).toContain('USE facts MATCH (a:Address)-[l:LINKED]-(b:Address)')

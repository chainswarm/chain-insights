...
describe('MCP graph report store', () => {
  ...
  beforeEach(async () => {
    fakeHome = join(tmpdir(), `ci-graph-reports-home-${Date.now()}-${Math.random().toString(36).slice(2)}`)
    workspace = join(tmpdir(), `ci-graph-reports-workspace-${Date.now()}-${Math.random().toString(36).slice(2)}`)
  ...
  it('writes canonical graph JSON under reports/graphs and returns a Hono graph report URL', async () => {
    ...
    await expect(stat(join(fakeHome, '.chain-insights', 'artifacts'))).rejects.toMatchObject({ code: 'ENOENT' })
  })
  ...
  it('rejects malformed graph arrays before writing a report file', async () => {
    ...
    await expect(stat(join(workspace, 'reports', 'graphs'))).rejects.toMatchObject({ code: 'ENOENT' })
    await expect(stat(join(workspace, 'artifacts'))).rejects.toMatchObject({ code: 'ENOENT' })
  })

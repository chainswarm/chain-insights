...
} from '../src/mcp/payment-asset-preference.js'
...
describe('orderPaymentOptions', () => {
  it('orders by preference symbols', () => {
    ...
    expect(ordered.map((o) => symbols[o.asset.toLowerCase() as keyof typeof symbols])).toEqual([
      'CHOICE',
      'USDC',
      'USDG',
    ])
  ...
  it('unknown preference entries are ignored', () => {
    ...
    expect(ordered.map((o) => symbols[o.asset.toLowerCase() as keyof typeof symbols])[0]).toEqual(
      'USDG'
    )
  })

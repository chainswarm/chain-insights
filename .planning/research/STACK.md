...
**Why DuckDB over SQLite/better-sqlite3:** This toolkit analyzes blockchain transaction data -- aggregations, joins across address histories, pattern detection. DuckDB's columnar engine is purpose-built for this. SQLite would require workarounds for every analytical query. DuckDB also reads Parquet natively, which is the standard format for blockchain data exports.

**Why DuckDB over StarRocks/Postgres:** Both require a separate server process. DuckDB embeds in-process, zero deployment friction. An investigator runs `npx chain-insights` and gets a working database immediately.
...
**Visualization architecture:** Fastify serves an HTML page containing the D3 visualization. The agent tells the local server "render this graph data" via an API call, then opens the URL in the user's browser. No Electron, no headless Chrome -- just HTML served locally.
...
**Payment flow for investigators:**
1. Investigator configures EVM private key in `.chain-insights/config`
2. Framework creates viem account: `privateKeyToAccount(privateKey)`
3. Framework registers with x402: `client.register("eip155:*", new ExactEvmScheme(signer))`
4. All MCP calls use wrapped fetch -- 402 responses handled automatically
5. USDC payments settle on Base (cheapest gas, Coinbase-hosted facilitator)
...
```bash
# Core dependencies
npm install fastify @duckdb/node-api viem @x402/fetch @x402/evm @x402/core \
  d3 jsdom open commander zod @modelcontextprotocol/sdk

# Dev dependencies
npm install -D typescript@6 tsdown tsx vitest @types/node @types/jsdom @types/d3
```
